...
with open("input2.yaml", 'r') as param_file:
    try:
    ...
    p.resetDebugVisualizerCamera(cameraDistance=dimensions[0] * 1.2, cameraYaw=0, cameraPitch=-89,
                                 cameraTargetPosition=[dimensions[0] / 2 -0.5, dimensions[1] / 2-0.5, 0])
...
cbs.main("input2.yaml", "output.yaml")
...
for i in agents:
    pos = [p.getBasePositionAndOrientation(i)[0][0], p.getBasePositionAndOrientation(i)[0][1], 2]
    p.loadURDF("sphere_small.urdf", pos)


with open("input3.yaml", 'r') as param_file:
    try:
        param = yaml.load(param_file, Loader=yaml.FullLoader)
        for i in param["agents"]:
            goals[boxId] = i["goal"]
